...
from src.dataset.generate import generate
...
def prepare(data_dir=Config.dataset_dir, n_samples=60000, eval_ratio=0.05):
    os.makedirs(data_dir, exist_ok=True)

    # 1. Generate data
    print(f"Generating {n_samples} samples...")
    generate(n_samples, eval_ratio)

    # 2. Read back all samples for tokenizer training
    texts = []
    for name in ["dataset/train.jsonl", "dataset/eval.jsonl"]:
    ...
    # Quick test
    test = f"{Config.start_token}user\nhi mudasir{Config.end_token}"
