...
class SelfInference:
    def __init__(self, checkpoint_path, tokenizer_path, device="cpu"):
        ...
        if os.path.exists(config_path):
            ...
            self.config = ModelConfig(**{k: v for k, v in ckpt["config"].items() if k in valid_fields})
        else:
            print("Warning: No config found, using defaults")
            self.config = ModelConfig()

        self.model = SelfLM(self.config).to(self.device)
        ...
        print(f"SelfLM loaded: {total/1e6:.1f}M params")
    ...
    def chat_completion(self, messages, temperature=0.7, max_tokens=64,
        ...
        # Truncate at first <|slm_end|> — don't let the model leak into the next turn
        if Config.end_token in output_text:
            output_text = output_text.split(Config.end_token)[0]
        # Also strip any <|slm_start|> fragments
        if Config.start_token in output_text:
            output_text = output_text.split(Config.start_token )[0]
    ...
    def _format_prompt(self, messages):
        ...
        for msg in messages:
            ...
            parts.append(f"{Config.start_token}{role}\n{content}{Config.end_token}")
        parts.append(f"{Config.start_token}assistant\n")
        return "\n".join(parts)


def chat():
    import argparse
    p = argparse.ArgumentParser(description="Chat with SelfLM")
    p.add_argument("--checkpoint", default="checkpoints/best_model.pt")
    p.add_argument("--tokenizer", default="dataset/tokenizer.json")
    p.add_argument("--device", default="cpu")
    p.add_argument("--prompt", "-p", help="Single prompt mode: ask one question and exit")
    args = p.parse_args()

    engine = SelfInference(args.checkpoint, args.tokenizer, args.device)
